function getEventStatus(wlurl=workloadfileURL,maineventSheet=mainEventsSheetName,defaultHeader=keyWord,lookupArray=eventTabLookupArray,trackerSheet=trackersSheetName) {
  ...
  trackerURLArray.forEach((trackerURL,row) => {
    ...
    if (trackerURL[0] !== "") {
      ...
      eventSheetHeaders.forEach((heading, ind) => {
        EventSheetObj[heading] = eventRow[ind]
      })
    }
    ...
    if (EventSheetObj["Event Status"] !== "Archived") {
