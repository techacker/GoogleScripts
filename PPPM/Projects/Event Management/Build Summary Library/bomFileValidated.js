function updateBOMFileWithValidation_(BuildObj, parentFileID) {
  ...
  */
  ...
  if (sheetNames.includes('BOM Summary')) {
    ...
    addNewDataWithRules_(parentFileID,'BOM Summary','Build Tracker URL',BuildObj)
    if (sheetNames.includes('Settings')) {
      ...
      reqdNames.forEach(names => {
        if (sheetNames.includes(names[0])) {
          addNewDataWithRules_(parentFileID,names[0],'Build Title',BuildObj)
