function updateSingleEventStatus(wlurl=buildsumfileURL,pbeBuildList=buildsListSheetName,defaultHeader=keyWord,lookupArray=buildListLookupArray,trackerSheet=buildDetailsSheetName, singleEventURL) {
  ...
  let trackerURLArray = eventSheet.getRange(headerRow+1, BuildListObj["Tracker URL"]+1, eventSheetLR-headerRow, 1).getValues(); 
  ...
  trackerURLArray.filter((trackerURL, row) => {
    if (trackerURL[0] === singleEventURL) {
      ...
      eventSheetHeaders.forEach((heading, ind) => {
        BuildListObj[heading] = eventRowData[ind]
      })
    }
  })
